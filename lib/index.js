...
class Service {
  constructor (options) {
    ...
    this.parseQuery = typeof options.parseQuery === 'function'
      ? options.parseQuery : (query) => query;
    this.aggregations = options.aggregations || {};
    this.aggregations.query = typeof this.aggregations.query === 'function'
      ? this.aggregations.query : () => undefined;
    this.aggregations.parse = typeof this.aggregations.parse === 'function'
      ? this.aggregations.parse : (aggs) => aggs;
  }
...
function find (service, params) {
  ...
  const aggsFlag = query.$aggs ? +query.$aggs === 1 || query.$aggs === 'true' : false;
  delete query.$aggs;
  let esQuery = parseQuery(service.parseQuery(query, service.id), service.id);
  let aggsQuery = aggsFlag ? service.aggregations.query(query, params) : undefined;
  ...
  return service.Model.search(findParams)
    ...
    .then(result => {
      result.aggregations = service.aggregations.parse(result.aggregations);
      return result;
    });
}
